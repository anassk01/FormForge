...
export class FormGeneratorComponent implements OnChanges {
  ...
  onSave(): void {
    if (this.formManagement.isFormValid(this.form, this.fields, this.isTemplate)) {
      const formData = this.formManagement.prepareFormData(this.form, this.fields);
      const structure = { entryName: this.entryName, fields: this.fields };
      
      if (this.isTemplate) {
        const defaultValues = FormUtils.getDefaultValues(this.fields);
  ...
  onSubmit(): void {
    if (this.form.valid) {
      const formData = this.formManagement.prepareFormData(this.form, this.fields);
      const structure = { entryName: this.entryName, fields: this.fields };
      this.formSubmitted.emit({ formData, structure });
    } else {
      this.showValidationError();
    }
  }
  ...
  private showValidationError(): void {
    ...
    // Utility methods
    getValidators = FormUtils.getValidators;
    getDefaultValue = FormUtils.getDefaultValue;
    createListFormArray = FormUtils.createListFormArray;
    createTableFormArray = FormUtils.createTableFormArray;
  ...
  private initializeTableData(): void {
    this.formManagement.initializeTableData(this.fields, this.form);
  }
